...
POBJ_LAYOUT_TOID(layout_name,
struct bz_node_block);
POBJ_LAYOUT_END(layout_name);

struct bz_node_block {
    POBJ_LIST_ENTRY(struct bz_node_block)
...
struct bz_memory_pool {
    ...
    void acquire(rel_ptr <rel_ptr<uint64_t>> ptr) {
        ...
        TOID(
        struct bz_node_block) front = POBJ_LIST_FIRST(&head_);
        if (!TOID_IS_NULL(front)) {
            ...
            TX_BEGIN(pop_)
            {
                pmemobj_tx_add_range_direct(ptr.abs(), sizeof(uint64_t));
                PMEMoid
                oid = pmemobj_tx_alloc(sizeof(bz_node_block) + NODE_ALLOC_SIZE, TOID_TYPE_NUM(
                struct bz_node_block));
    ...
    void release(rel_ptr <rel_ptr<uint64_t>> ptr) {
        ...
        TOID(
        struct bz_node_block) back = TOID(
        struct bz_node_block)(oid);
